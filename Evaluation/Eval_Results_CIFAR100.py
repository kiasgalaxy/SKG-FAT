...
from models_CIFAR100.preact_resnet import PreActResNet18
import tqdm
...
def get_args():
    ...
    parser.add_argument('--model_select', default='Res18', type=str, choices=['Res18', 'PRN'])
    parser.add_argument('--n_ex', type=int, default=10000)
    parser.add_argument('--batch_size', type=int, default=128)
    parser.add_argument('--out_dir', type=str, default='Results/Evaluation_Results')
...
if args.model_select == 'Res18':
    target_model = ResNet18()
elif args.model_select == 'PRN':
    target_model = PreActResNet18()

target_model.load_state_dict(torch.load(full_path), strict=False)
...
logger.info('Clean \t PGD-10 \t PGD-20 \t PGD-50 \t CW \t AA')


pgd10_result = eval_pgd10(target_model ,test_loader, epsilon, alpha)
pgd20_result = eval_pgd20(target_model ,test_loader, epsilon, alpha)
...
pgd_20_acc = pgd20_result[1]
pgd_50_acc = pgd50_result[1]
cw_acc = cw_result[1]
AA_acc = AA_result[1]

logger.info(args.model_path)
logger.info('%.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f', clean_acc, pgd_10_acc, pgd_20_acc, pgd_50_acc, cw_acc, AA_acc)


# pgd10_result = eval_pgd10(target_model ,test_loader, epsilon, alpha)
# clean_acc = pgd10_result[0]
# pgd_10_acc = pgd10_result[1]
# logger.info('%.4f \t %.4f', clean_acc, pgd_10_acc)

# AA_result = eval_AutoAttack(target_model, test_loader, epsilon)
# clean_acc = AA_result[0]
# pgd_10_acc = AA_result[1]
# logger.info('%.4f \t %.4f', clean_acc, pgd_10_acc)