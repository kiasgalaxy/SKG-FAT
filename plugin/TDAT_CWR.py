...
output_path = os.path.join(args.out_dir, f'TDAT_CWR')
        ...
        if x.shape[0] == args.batch_size:
            ...
            for i in range(num_classes):
                ...
                if class_indices.numel() > 0:
                    C_output = output[class_indices]
                    R_output = output_adv[class_indices]
...
def train(args, model, train_loader, opt, scheduler, epoch, init_momentum):
    ...
    lr = scheduler.get_lr()[0]
    logger.info('Epoch \t LR \t Train normal Loss  \t Train Loss \t Train Acc')
    logger.info('%d \t %.4f \t %.4f \t %.4f', epoch, lr, train_loss / train_n, train_acc / train_n)
    return train_loss/train_n, init_momentum, train_logger.result()
...
def main():
    ...
    for epoch in range(args.epochs):
        if epoch < 1:
            ...
            train_loss, init_momentum, train_log = train_CWR(args, target_model, train_loader, opt, scheduler, epoch, init_momentum, Class_Cleanacc)

        if epoch < 10 or epoch > args.epochs-15:
            if args.data == 'CIFAR10':
