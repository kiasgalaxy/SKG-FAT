...
def main():
    args = get_args()
    output_path = os.path.join(args.out_dir, f'MEP_AGR')
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    logfile = os.path.join(output_path, 'train_info.log')
    ...
    logging.basicConfig(
        ...
        datefmt='%Y/%m/%d %H:%M:%S',
        level=logging.INFO,
        filename=os.path.join(output_path, 'train_info.log'))
    logger.info(args)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < 1:
            ...
            epoch_acc = CWlogger.result()
        else:
            pre_epoch_acc = epoch_acc
            Avg_Cleanacc = pre_epoch_acc[0]
            Avg_Robustacc = pre_epoch_acc[1]
            Class_Cleanacc = pre_epoch_acc[2]
            Class_Robustacc = pre_epoch_acc[3]
